...
For platforms that follow the POSIX standard, ALL calls BUT those
listed below are supposed to be multi-thread safe.  In the list below:

=over

=item C<÷> means that Perl furnishes a thread-safe version in the POSIX
modue that you should use instead, using the methods given in
L<perlcall>.

=item C<@> and C<@@> mean for function I<foo>, there is an equivalent function
I<fooC<_r>>, available on some platforms, whose result is returned in a
caller-supplied space, and which is thread-safe.  C<@@> means the
function is specified in the Posix standard, so it should be available
on most platforms.  A single C<@> indicates that it is an extension to
Posix, and so support for it may be more spotty.

If you use the C<"I<foo>_r"> form instead of plain I<foo>, your code should
be thread-safe with respect to this operation, unless your platform is
buggy.

=item C<£> and C<££> are like C<@> and C<@@> respectively, but there are
known platforms where the C<_r> version is thread-safe only if the
program's global locale is not changed by another thread in the middle
of this function's execution.  Starting in Perl 5.28, this should not be
an issue on platforms that have implemented the POSIX 2008 per-thread
locale functionality.

=item C<¤> is like C<@>, but this function accesses the program's
environment, and the C<_r> form is thread-safe only if the program's
environment is not changed by another thread in the middle of this
function's execution.  These functions should be protected by

 LOCK_ENV
 ...
 UNLOCK_ENV

calls surrounding them.  This protection is illusory if other code
changes the environment without using the same locking mechanism.  The
perl core does use this mechanism, so any conflicts would have to come
from another thread that uses XS.

=item C<×> means that these functions should not be generally be used in
a multi-thread environment, unless only a single thread is calling them.
There may be C<_r> equivalents defined for them, but these are still
unsafe.

Some of these iterate over a data strtucture on a per-process basis, NOT
...
cannotis like C<@>, but there are known platforms that are buggy
and the C<"_r"> forms are still not-thread safe due to some race
condition.  These XXX WRONG can be made thread-safe by using a mutex to keep them
from being interrupted, as described below in L<Making thread-safe code>.
#
...
Should change reentr.pl to handle the _r ones listed below that perl
currently doesn't do, like gmtime, localtime

 asctime()  ££ ¶    XXX use strftime() POSIX 2008 says
 basename()
 catgets()
 crypt()  @ ¶
 ctermid() if passed a non-NULL argument @ ¶
 ctime()  ££ ¤ ¶    XXX use strftime() POSIX 2008 says
 dbm_clearerr()
 dbm_close()
 dbm_delete()
 dbm_error()
 ...
 lgammaf()  @
 lgammal()  @
 localeconv()  ÷ (for the most common uses, instead use Perl_langinfo() for thread-safety
                  starting with Perl 5.28, or call the POSIX)
 localtime()  ££ ¤
 lrand48()
 mblen()  ÷ or use mbrlen()
 mbrlen()     if its 'ps' argument is NULL
 mbrtowc()    if its 'ps' argument is NULL
 mbsnrtowcs() if its 'ps' argument is NULL
 mbsrtowcs()  if its 'ps' argument is NULL
 mbtowc()  ÷ or use mbrtowc()
 mrand48()
 nftw()
 nl_langinfo()  (instead use Perl_langinfo() for thread-safety
                 starting with Perl 5.28)
 ptsname()  @
 putchar_unlocked() unless locked by flockfile() or ftrylockfile()
 putc_unlocked()    unless locked by flockfile() or ftrylockfile()
 putenv()  ¤
 pututxline()
 rand()  @@
 readdir()  @@ ¶
 ...
 setgrent()  × ¶
 setkey()  @
 setlocale()  (instead use Perl_setlocale() for thread-safety starting
               with Perl 5.28)
 setnetent()  × ¶
 setpwent() ¶
 setutxent()
 ...
 unsetenv()
 usleep()
 wcrtomb()    if its 'ps' argument is NULL
 wcsnrtombs() if its 'ps' argument is NULL
 wcsrtombs()  if its 'ps' argument is NULL
 wcstombs()  ÷ or use wcsrtombs()
 wctomb()  ÷   or use wcrtombs()

(The list is taken from the POSIX 2001
and 2008
...
The above list should be taken as just a starting point, as it is not
entirely correct, for several reasons.
...
threads.  That cooperation doesn't take much coding work, as described
below in L</Making thread-safe code>, but the fact remains that one bad
actor can create a problem.  And you might have trouble convincing the
maintainers of such code to fix it.  If available, a reentrant version
of the function should be used instead.  Perl does this automatically
for many functions if F<perl.h> is C<#include>'d in your code.  The file
F<regen/reentr.pl> shows all the ones perl currently knows about, not
all of which are likely to be available on your platform.
...
=head2 Making thread-safe code

yield execution until it is complete, and any data it returns that is
common to the whole program is acted upon or copied to a safe area.

XXX not yet, and this was written under false assumptions
Perl furnishes macros to do this:

 MT_LOCK;
 result = MT_savepv(getpwent());
 MT_UNLOCK;

This makes C<getpwent()> thread-safe.  C<result> needs to be freed at
some point to prevent a memory leak; you can use either

 MT_Safefree(result)

or

 MT_SAVEFREEPV(result)

depending on your needs. (C<MT> stands for multi-threaded.)

L<C<MT_LOCK>|perlapi/MT_LOCK> starts a critical section, ended by
L<C<MT_UNLOCK>|perlapi/MT_UNLOCK>, during which only this thread will
execute.  It should be kept as short as possible to minimize performance
issues.  On single-threaded builds, no critical sections are needed, and
these macros become no-ops.

In multi-threaded builds, L<C<MT_savepv>|perlapi/MT_savepv> expands to
L<C<savepv>|perlapi/savepv>; but on single-threaded ones it is just a
no-op.  Same with the rest of the macros.  In this way, you can write
code that works properly on multi-threaded builds, but doesn't do
unnecessary things on single-threaded.

Of course, you may wish to save the result somewhere else, such as
specified in L</Safely Storing Static Data in XS>, so you don't have to
save things like the above example suggests.

Starting from Perl 5.8, Perl can automatically changes calls to
non-thread-aware functions into the reentrant versions (whose names end
in C<"_r"> that are available on the system.  It does this by #defining
a macro (I<e.g.> getpwnam()) that instead calls the thread-aware
equivalent (C<getpwnam_r()>).  Unfortunately something that is
thread-aware may not be thread-safe on a given system, in violation of
the POSIX standard.  An example is L<C<getpwent(3)>>, which on some
systems is not thread-safe.  The only thing that can be done is to wrap
calls to things like this in mutexes, as described in the example above
for C<getpwent>.

This wrapping by Perl happens always when compiling Perl core source
or the Perl core extensions (when PERL_EXT is #defined).  When compiling XS
code outside of Perl core the wrapping does not take place.  Note,
however, that intermixing the C<_r>-forms (as Perl compiled for
multithreaded operation will do) and the C<_r>-less
...
Some of the unsafe functions have safe versions in the L<POSIX> module,
and accessible from XS code via the methods given in L<perlcall>.
